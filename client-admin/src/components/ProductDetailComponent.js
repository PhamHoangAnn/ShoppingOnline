...
class ProductDetail extends Component {
 ...
 apiGetCategories() {
    ...
    axios.get('/api/admin/categories', config).then((res) => {
      ...
      this.setState({ categories: Array.isArray(result) ? result : [] });
    }).catch(() => {
 ...
 apiPostProduct(prod) {
  ...
  apiGetProducts() {
    ...
    axios.get('/api/admin/products?page=' + this.props.curPage, config).then((res) => {
      const result = res.data;
      if (!result || result.success === false || !Array.isArray(result.products)) {
        return;
      }
      if (result.products.length !== 0) {
        ...
        axios.get('/api/admin/products?page=' + curPage, config).then((res2) => {
          ...
          if (result2 && Array.isArray(result2.products)) {
            this.props.updateProducts(result2.products, result2.noPages, curPage);
          }
        });
      }
    });
